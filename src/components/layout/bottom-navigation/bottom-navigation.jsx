...
export default function BottomNavigation({ children }) {
	...
	const { users, loading, searchUsers, GetHystoryMobile, AddSearchMobile } =
		useUserStore()
	...
	const iconClass =
		'flex items-center gap-4 rounded-[8px] h-[52px] relative px-0 m-[0] justify-center'
		...
		<>
			...
			<section className='fixed w-[100%] z-[10] bottom-0'>
				<div className='flex gap-[0.5rem] mt-4 align-bottom bg-white dark:bg-gray-900 justify-evenly'>
					...
					<button onClick={toggleDrawer(true)}>{searchIcon}</button>
						...
						<Link href='/profile'>
							<div className={iconClass}>
								<Image
									className={`${
										pathname === '/profile'
											? 'border-[2px] border-solid border-[black] dark:border-white'
											: ''
									} ${profileClass}`}
									src={
										data
