...
import { Settings, BookmarkBorder, Logout } from '@mui/icons-material'
...
export default function SideBar({ children }) {
  ...
  const { users, loading, searchUsers, GetHystory, AddSearchHystory } = useUserStore()
  ...
  const DrawerList = (
    <Box className="p-4 dark:bg-gray-900 dark:text-gray-200" role="presentation">
      <div>
        ...
        <div className="relative w-full">
          ...
          {showDropdown && query && (
            <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-900 border dark:border-gray-700 rounded-md shadow-lg max-h-60 overflow-y-auto z-50">
              {loading && (
                <p className="p-2 text-gray-500 dark:text-gray-400">{t('layout.loading')}</p>
              )}
              {!loading && users.length === 0 && (
                <p className="p-2 text-gray-500 dark:text-gray-400">{t('layout.noResults')}</p>
              )}
              {users.map((user) => (
                <Link
        ...
        <div className="sideBar h-full pb-[100px] dark:bg-gray-900">
          ...
          <div className="flex flex-col justify-between h-full">
            <div className="flex flex-col gap-2 mt-4">
              ...
              <Link href="/profile" className={`flex items-center gap-4 w-[90%] m-auto rounded-md h-[52px] px-4 hover:bg-gray-100 dark:hover:bg-gray-800 ${isActive('/profile')}`}>
                <Avatar
                  src={data?.image ? `http://37.27.29.18:8003/images/${data?.image}` : '/default-avatar.png'}
                  alt={data?.userName || 'Profile'}
                  sx={{ width: 25, height: 25 }}
                />
                <p className="text-lg dark:text-gray-200">{t('layout.profile')}</p>
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-2 mb-4">
              ...
              <div className="flex items-center gap-4 w-[90%] m-auto rounded-md h-[52px] px-4 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                <button onClick={handleClick} className="flex gap-3 items-center">
                  <MenuIcon className="text-gray-700 dark:text-gray-300" />
                  <p className="text-lg text-gray-800 dark:text-gray-200 font-medium">{t('layout.more')}</p>
                </button>
                ...
                <Menu
                  ...
                  PaperProps={{ sx: { borderRadius: '12px', minWidth: '220px', boxShadow: '0px 4px 20px rgba(0,0,0,0.1)' } }}
                >
                  <Link href="/setting">
                    <MenuItem onClick={handleCloseMenu} className="flex gap-3 items-center text-gray-700 dark:text-gray-200 dark:bg-gray-900">
                      <Settings fontSize="small" /> {t('layout.settings')}
                    </MenuItem>
                  </Link>

                  <Link href="/profile/saved" passHref>
                    <MenuItem onClick={handleCloseMenu} className="flex gap-3 items-center text-gray-700 dark:text-gray-200">
                  ...
                  <MenuItem sx={{ color: '#ed4956', fontWeight: 600 }} className="flex gap-3 items-center" onClick={logOut}>
