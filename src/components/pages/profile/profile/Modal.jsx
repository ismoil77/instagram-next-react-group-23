...
import { Modal, Box, IconButton, Typography, TextField, Button, Tooltip } from "@mui/material"
import { X, Heart, MessageCircle, Volume2, VolumeX, Trash2, ChevronLeft, Ellipsis, Send } from "lucide-react"
import Image from 'next/image'
import { Swiper, SwiperSlide } from 'swiper/react'
import { Mousewheel, Navigation, Pagination } from 'swiper/modules'
import "swiper/css"
import { useFav, usePosts } from '@/store/pages/profile/profile/store-profile'
import { useMy_Profile } from '@/store/pages/profile/profile/store-profile'
...
const Reels = ({ open, onClose, posts, startIndex }) => {
	...
	const { MYsubscribtions, followUser, dontFollowUser, updateSubscriptionState } = useMy_Profile()
	const isFollowing = MYsubscribtions?.some(s => s?.userShortInfo?.userId === currentPost?.userId)
	...
	const toggleFavorite = isFromSaved ? favStore.add_post_favorite : postsStore.add_post_favorite
	const addComment = isFromSaved ? favStore.add_comment : postsStore.add_comment

	const handleFollowToggle = async () => {
		if (!currentPost?.userId) return

		const currentlyFollowed = MYsubscribtions?.some(s => s?.userShortInfo?.userId === currentPost?.userId)
	...
	const handleUser = async () => {
		try {
			const { data: { data } } = await axiosRequest.get(`http://37.27.29.18:8003/UserProfile/get-user-profile-by-id?id=${currentPost?.userId}`)
			setUser(data)
		} catch (error) {
	...
	return (
		<>
			<Modal open={open} onClose={onClose} className='md:w-[80%] md:h-[80%] md:flex hidden items-center m-auto justify-center'>
				<Box className="md:flex hidden items-center justify-center h-full w-full bg-black/50">
											...
											{i.endsWith(".mp4") ? (
												<div className="w-full h-full relative">
													<video
														...
														src={"http://37.27.29.18:8003/images/" + i}
					...
					<div className="md:flex hidden h-full flex-col justify-between md:p-[20px] md:w-1/2 bg-white dark:bg-[#121212] text-black dark:text-white">
						<div className="flex items-center gap-[20px]">
							...
							<Button
								...
								variant="text"
								className={`font-bold ${isFollowing ? 'text-gray-500 dark:text-[#A8A8A8]' : 'text-blue-500 dark:text-white'}`}
						...
						<div style={{ scrollbarWidth: "none" }} className="flex flex-col gap-5 overflow-y-auto h-[60vh]">
							{currentPost?.comments?.length ? (
								currentPost?.comments.map((c) => (
									<div key={c?.postCommentId} className="flex flex-col gap-1">
										<div className="flex items-start flex-col gap-2">
											<div className="flex items-center gap-3">
												<Link href={`/profile/${c?.userId}/posts`} className='w-7 h-7 rounded-full relative'>
													<Image
														src={
															c?.userImage
												...
												<div className='flex flex-col'>
													<Link href={`/profile/${c?.userId}/posts`} className="font-semibold leading-3 text-sm text-black dark:text-white">{c?.userName}</Link>
													{c?.dateCommented && (
														<div>
															<span className="text-[10px] leading-0 text-gray-500 dark:text-[#A8A8A8]">
																{new Date(c?.dateCommented).toLocaleDateString("ru-RU", {
																	day: "numeric",
																	month: "long",
																})}
															</span>
											...
											<p className="text-sm break-words text-black dark:text-white" style={{ wordBreak: "break-word" }}>{c?.comment}</p>
										</div>
									</div>
								))
							) : (
								<Typography variant="body2" className="text-gray-500 dark:text-[#A8A8A8]">
						...
						<div className="flex flex-col gap-3">
							<div className="flex items-center justify-between">
								<div className="flex gap-4">
									...
									<IconButton className="p-0">
										...
										<p className='text-base text-black dark:text-white'>{currentPost?.commentCount}</p>
									</IconButton>
									<IconButton className="p-0">
										{pathname !== "/profile/posts" && pathname !== "/profile" ? <svg
										...
										>
											...
											/>
										</svg> : <Trash2 onClick={() => { postsStore.delete_post(currentPost?.postId), onClose() }} className='text-red-500' />}
									</IconButton>
								</div>
							...
							<div className="flex items-center w-full justify-between">
								...
								>
									<Send className={`${comment.trim() ? 'text-blue-500 dark:text-white' : 'text-blue-500 dark:text-white opacity-50'}`} />
								</Button>
							</div>
						</div>
					</div>
					...
					<button
						...
						onClick={() => { onClose(), favStore.getFav() }}
					>
						...
						{posts?.map((post, idx) => (
							<SwiperSlide key={post.postId} className=''>
								<div className='w-full h-screen flex items-center justify-center bg-black'>
									{post?.images?.length > 1 ? <Swiper modules={Pagination} pagination={{ clickable: true }} spaceBetween={0} slidesPerView={1}>
										{post.images.map((img, i) => (
											<SwiperSlide key={i} className='w-full h-screen flex items-center justify-center bg-black'>
												{img.endsWith(".mp4") ? (
													<video
													...
													<div
														...
														style={{
															backgroundImage: `url(http://37.27.29.18:8003/images/${img})`,
									...
									</Swiper> : <>{post.images[0].endsWith(".mp4") ? (
										<video
											ref={el => videoRefs.current[idx] = el}
										...
										<div
											className="w-full h-full bg-black flex items-center z-40 justify-center"
											style={{
												backgroundImage: `url(http://37.27.29.18:8003/images/${post.images[0]})`,
												backgroundSize: 'contain',
												backgroundPosition: 'center',
												backgroundRepeat: 'no-repeat',
											}}
									...
									<div className='absolute top-6 z-40 right-6'>
										...
										{focused &&
											<div className='absolute top-8 right-0 bg-white dark:bg-[#262626] text-black dark:text-white flex flex-col rounded shadow-lg'>
												{pathname === "/profile/posts" && <p onClick={() => { postsStore?.delete_post(post.postId), onClose(), postsStore?.get_my_Posts() }} className="px-3 py-1 hover:bg-gray-100 dark:hover:bg-[#363636] rounded-lg flex gap-2">
													{(<><Trash2 className="text-red-500 w-6 h-6" fill="currentColor" /> <p className='text-red-500 dark:text-red-400'>Удалить</p></>)}
												</p>}
											</div>}
									</div>
									<div className="absolute bottom-6 right-4 flex flex-col gap-4 z-40">
										<IconButton onClick={handleLike} className="p-0 flex flex-col">
											{currentPost?.postLike ? (
												<Heart className="text-red-500 w-6 h-6" fill="currentColor" />
											) : (
												<Heart className="text-white w-6 h-6" />
											)}
											<p className='text-white text-sm'>{currentPost?.postLikeCount}</p>
										</IconButton>

										<IconButton onClick={() => { setCommentaries(true) }} className="p-0 flex flex-col">
											<MessageCircle onClick={() => { setCommentaries(true) }} className="text-white w-6 h-6" />
											<p className='text-base text-white'>{currentPost?.commentCount}</p>
										</IconButton>

										<IconButton onClick={handleSave} className="p-0 flex flex-col">
											{currentPost?.postFavorite ? (
												<svg
													xmlns="http://www.w3.org/2000/svg"
													fill="currentColor"
											...
											<p className='text-white text-sm'>{currentPost?.userFavorite?.length}</p>
										</IconButton>

										{post.images[0].endsWith(".mp4") && <IconButton className="p-0"
											onClick={() => {
									...
									<div className="absolute bottom-6 left-4 w-[85%] text-white z-40">
										...
										<p className="text-sm wrap-break-word opacity-80">{post?.content}</p>
									</div>

									<div onClick={() => { setCommentaries(false) }} className={`${commentaries ? "h-[40vh] top-0 bottom-0" : "h-0 bottom-0 -left-72"} w-full bg-transparent absolute transition-all duration-200 z-[100]`} />
									<div className={`${commentaries ? "h-[60vh] bottom-0 left-0" : "h-0 bottom-0 -left-72"} w-full bg-white dark:bg-[#121212] rounded-t-lg absolute transition-all duration-200 z-[100]`}>
										<div className={`w-full ${!commentaries && "hidden"} relative border-b border-gray-300 dark:border-[#363636] py-3 px-5 flex justify-end`}>
											<p className='absolute top-2 left-1/2 -translate-x-1/2 text-lg font-semibold text-black dark:text-white'>{commentaries && "Comments"} {currentPost?.commentCount?.length > 0 && currentPost?.commentCount}</p>
											<X onClick={() => setCommentaries(false)} className='text-black dark:text-white w-6 h-6 cursor-pointer' />
										</div>
										<div className={`w-full ${!commentaries && "hidden"} h-[46vh] overflow-y-scroll py-5 px-[5%]`}>
											{currentPost?.comments?.length > 0 ? (
												Array.from(currentPost?.comments || [])?.reverse().map((c) => (
													<div key={c?.postCommentId} className="flex flex-col gap-1">
														<div className="flex items-start flex-col gap-2">
															<div className="flex items-center gap-3">
																<Link href={`/profile/${c?.userId}/posts`} className='w-7 h-7 rounded-full relative'>
												...
												<Typography variant="body2" className="text-gray-500 dark:text-[#A8A8A8] text-center">
													Комментариев пока нет
												</Typography>
											)}
										</div>
										<form className={`w-full ${!commentaries && "hidden"} h-[7vh] px-[5%] border-t border-gray-300 dark:border-[#363636] p-2 flex`} onSubmit={handleComment_2}>
											<input
												type="text"
												name='comment'
												placeholder='Comment...'
