...
export default function SavedPage() {
	...
	return (
		<div className="flex flex-col items-center gap-5 w-full h-full">
			{reversedFavourite?.length > 0 ? <div className="grid grid-cols-3 gap-[1px] md:gap-3 w-full">
				{reversedFavourite?.map((i, idx) => (
					<div key={idx}>{i?.images[0]?.endsWith(".mp4") ? (<div className='relative w-fit aspect-square'>
						<video
							onClick={() => { setStart(idx), setOpen(true) }}
				...
				<div className='flex items-center py-32 w-full flex-col'>
					<div className='relative w-[150px] h-[200px]'>
						<Image src={image78} fill className='object-cover' alt='empty'></Image>
					</div>
					<p>Your favourites is empty</p>
				</div>}
			<Reels open={open} posts={reversedFavourite} startIndex={start} onClose={() => setOpen(false)} />
		</div>
	)
}