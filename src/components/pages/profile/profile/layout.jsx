...
import { Bookmark, Camera, Plus, Settings } from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'
import { useParams, usePathname } from 'next/navigation'
import { useEffect, useState } from 'react'
import { StoriesViewer } from '../story/story'
								...
								>
									{u?.userPhoto ? (
										<div className='w-10 h-10 relative'>
											<Image
												src={`https://instagram-api.softclub.tj/images/${u?.userPhoto}`}
												alt=''
												fill
...
export default function ClientProfile({ children }) {
	...
	const toggleFollow = async userId => {
		const currentlyFollowed = MYsubscribtions?.some(
			s => s?.userShortInfo?.userId === userId
		)
		try {
				...
				{!(profileLoading && !data) && (
					<div className='flex flex-row items-center md:px-0 px-[5%] gap-6 w-full h-[160px]'>
						...
						<div
							...
							className={`${
								stories?.length > 0
									? 'p-1 relative cursor-pointer bg-gradient-to-tr from-purple-500 via-pink-500 to-blue-500 rounded-full'
									: ''
							}`}
						>
							<div
								className={`rounded-full relative md:size-[160px] size-[100px] dark:border-black border-white object-cover ${
									stories?.length > 0 ? 'border-4' : ''
								}`}
							>
								<Image
									src={
							...
							{checkPath ? (
								<div className='flex gap-3'>
									<Link
										...
										className='px-4 py-2 truncate rounded-lg bg-gray-300 dark:bg-[#262626] text-black dark:text-white font-semibold text-sm leading-5'
									>
										Edit Profile
									</Link>
								</div>
								...
								<div className='flex gap-3'>
									<button
										...
										className='px-4 py-2 truncate rounded-lg bg-blue-500 text-white font-semibold text-sm leading-5'
									>
										Follow
									</button>
								</div>
