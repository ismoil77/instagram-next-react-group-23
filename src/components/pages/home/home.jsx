...
const PostCardHomePage = ({ post, giveId, onOpen }) => {
  ...
  const isVideo = !!currentMedia && currentMedia.endsWith && currentMedia.endsWith('.mp4')
  ...
  useEffect(() => {
    if (!isClient) return
    if (post?.content) {
      setDescFull(post.content.length > 20 ? post.content.slice(0, 20) + '...ещё' : post.content)
    }
    try {
  ...
  return (
    ...
    >
      ...
      {/* Верхняя часть поста: аватар и имя */}
      <div className='flex items-center mb-3'>
        {stories.some(el => el.userId === post.userId) ? (
          <div className='w-11.5 h-11.5 bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 rounded-full flex justify-center items-center'>
            <img
              src={post?.userImage ? `${API_IMAGE}/${post.userImage}` : "https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_640.png"}
              alt={post.userName}
              width={40}
              height={40}
              className='rounded-full w-10 h-10 object-cover bg-amber-50 dark:bg-gray-800 border'
              onClick={() => { onOpen(); giveId(post.userId) }}
            />
          </div>
        ) : (
        ...
        <div className='ml-3 flex-1'>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400'>
            {post.datePublished ? new Date(post.datePublished).toLocaleDateString('ru-RU') : ''} {post.country || 'Таджикистан'}
          </p>
        </div>
        <button
      ...
      <div className='relative h-[600px] w-[520px] max-sm:w-full aspect-video mb-3 bg-black rounded-lg border border-slate-300 dark:border-gray-600'>
        ...
        {isVideo && (
          <>
            ...
            >
              <img
                src={isMutedVoice ? 'https://www.svgrepo.com/show/74628/speaker-sound-muted.svg' : 'https://images.icon-icons.com/2645/PNG/512/volume_up_icon_159763.png'}
                alt='звук'
                className='w-5 h-5'
              />
            </button>
        ...
        {images.length > 1 && (
          <>
            <button onClick={handlePrevMedia} className='absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full'>←</button>
            <button onClick={handleNextMedia} className='absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full'>→</button>
            <div className='absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-1'>
              {images.map((_, index) => (
                <button
                  key={index}
                  onClick={() => setCurrentMediaIndex(index)}
                  className={`w-2 h-2 rounded-full ${index === currentMediaIndex ? 'bg-white' : 'bg-gray-500 dark:bg-gray-400'}`}
                />
              ))}
            </div>
          </>
      ...
      {/* Кнопки лайков, комментариев, шера, избранного */}
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center space-x-4'>
          <button onClick={handleLikeClick} className='text-gray-500 dark:text-gray-300 hover:text-red-500 transition-colors'>
            <svg xmlns='http://www.w3.org/2000/svg' fill={post.postLike ? 'red' : 'none'} stroke='currentColor' viewBox='0 0 24 24' className='w-6 h-6'>
              <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z' />
            </svg>
          </button>

          <button onClick={() => setCommentOpenAll(!commentOpenAll)} className='text-gray-500 dark:text-gray-300 hover:text-blue-500 transition-colors'>
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' stroke='currentColor' viewBox='0 0 24 24' className='w-6 h-6'>
              <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.338-3.127A7.5 7.5 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' />
            </svg>
          </button>

          <button className='text-gray-500 dark:text-gray-300 hover:text-green-500 transition-colors' onClick={() => setShareModalOpenClose(!shareModalOpenClose)}>
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' stroke='currentColor' viewBox='0 0 24 24' className='w-6 h-6'>
        ...
        <button onClick={handleFavoriteClick} className='text-gray-800 dark:text-gray-200 hover:text-black dark:hover:text-white transition-colors'>
          <svg xmlns='http://www.w3.org/2000/svg' fill={post.postFavorite ? 'black' : 'none'} stroke='currentColor' viewBox='0 0 24 24' className='w-6 h-6'>
            <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M5 5a2 2 0 012-2h10a2 2 0 012 2v14l-5-2.5L5 19V5z' />
          </svg>
        </button>
      </div>
      ...
      <div className='text-sm text-gray-600 dark:text-gray-400 mb-2'>
        ...
        {post.commentCount > 1 && <span className='mx-1'>• {post.commentCount} комментарий{post.commentCount !== 1 ? 'ев' : ''}</span>}
      </div>
      ...
      {post.commentCount > 0 && (
        <>
          ...
          >
            {commentOpenAll ? 'Скрыть комментарии' : `Посмотреть все ${post.commentCount} комментариев`}
          </p>

          {(commentOpenAll ? post.comments : post.comments.slice(0, 1))?.map(comment => (
            <div key={comment.postCommentId} className='flex items-start mb-1 text-sm text-gray-700 dark:text-gray-300'>
              <img
                src={comment.userImage ? `${API_IMAGE}/${comment.userImage}` : "https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_640.png"}
      ...
      {/* Дата */}
      <p className='text-xs text-gray-400 dark:text-gray-500 mt-2'>{timeAgo(post.datePublished)}</p>
    </div>
  )
}
