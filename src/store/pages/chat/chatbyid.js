...
export const useChatStore2 = create((set) => ({
  ...
  getChatById: async (chatId) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://37.27.29.18:8003/Chat/get-chat-by-id?chatId=${chatId}`,
        {
          method: "GET",
          headers: {
            accept: "*/*",
  ...
  sendMessage: async ({ chatId, messageText, file }) => {
    ...
    try {
      ...
      const res = await fetch(`http://37.27.29.18:8003/Chat/send-message`, {
        method: "PUT",
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
  ...
  deleteMessage: async (messageId) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://37.27.29.18:8003/Chat/delete-message?massageId=${messageId}`,
        {
          method: "DELETE",
          headers: {
            accept: "*/*",
      ...
      set((state) => ({
        chatById: state.chatById?.filter(
