...
const EditProfile = () => {
	...
	useEffect(() => {
		if (data) {
			setName(`${data.firstName || ''} ${data.lastName || ''}`.trim() || data.userName || '')
	...
	const handleSubmitProfile = async (e) => {
		...
		try {
			const response = await axiosRequest.put('http://37.27.29.18:8003/UserProfile/update-user-profile', {
				about: bio,
				gender: gender,
			})
	...
	const handleSubmitImage = async () => {
		...
		setIsUploading(true)
		try {
			const response = await axiosRequest.put('http://37.27.29.18:8003/UserProfile/update-user-image-profile', formData, {
				headers: {
	...
	const getAvatarUrl = () => {
		...
		if (data?.image) return `http://37.27.29.18:8003/images/${data.image}`
		return Profile
	}
	...
	if (getMyProfileLoading && !data) {
		return (
			<div className="min-h-screen bg-gray-50 dark:bg-[#121212] p-6 flex items-center justify-center">
				<div className="text-gray-500 dark:text-[#A8A8A8]">Loading profile...</div>
			</div>
		)
	}
		...
		<div className="min-h-screen bg-gray-50 dark:bg-[#121212] p-6">
			<div className="max-w-3xl mx-auto">
				...
				<div className="flex items-center space-x-2 text-sm font-medium mb-6">
					<Link href="/profile/posts" className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Profile</Link>
				...
				<div className="bg-gray-100 dark:bg-[#262626] rounded-xl p-5 mb-6 flex items-center justify-between">
					<div className="flex items-center space-x-4">
						...
						<div>
							...
							<p className="text-gray-600 dark:text-[#A8A8A8] text-sm">{name}</p>
						</div>
					</div>
					<div className='flex gap-5'>
						...
						<input type="file" name="imageFile" onChange={(e) => { setAvatar(e.target.files[0]) }} className="hidden" id="upload" />
						{avatar && <button
							...
							onClick={handleSubmitImage}>{isUploading ? "Uploading..." : "Submit changes"}</button>}
					</div>
				</div>
				...
				<form onSubmit={handleSubmitProfile} className="space-y-4">
					{/* Name */}
					<div>
						<label className="block text-xs text-gray-500 dark:text-[#A8A8A8] font-medium mb-1">Name</label>
						<input
							type="text"
							value={name}
					...
					{/* Username */}
					<div>
						<label className="block text-xs text-gray-500 dark:text-[#A8A8A8] font-medium mb-1">User name</label>
						<input
							type="text"
							value={username}
					...
					{/* Bio */}
					<div>
						<label className="block text-xs text-gray-500 dark:text-[#A8A8A8] font-medium mb-1">Bio</label>
						<textarea
							value={bio}
					...
					{/* Gender */}
					<div>
						<label className="block text-xs text-gray-500 dark:text-[#A8A8A8] font-medium mb-1">Gender</label>
						<select
							value={gender}
					...
					<button
						type="submit"
						disabled={isSubmitting}
						className={`w-full py-3 px-6 rounded-lg text-sm font-medium transition-colors duration-200 ${isSubmitting
