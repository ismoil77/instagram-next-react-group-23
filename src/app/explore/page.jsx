"use client"
import { cancelIcon, commentIcon, emojiIcon, emptyLike, emptySaveIcon, heartIcon, leftIcon, moreIcon, muteOff, muteOn, oneMoreImageIcon, savedIconn, sendIcon, solidSavedIcon, userIcon, videoIcon } from "@/assets/iconsExplore";
...
export default function Explore() {
    ...
    const { data, GetExplore, postLike, addComment, GetUsers, toggleFavorite, followUser } = useDataExplore((state) => state);
    ...
    const showInfoModal = async (post) => {
        setInfoModal(true);
        setCommentPost(post);
        const files = post.images.map(img => `http://37.27.29.18:8003/images/${img}`);
        setMedia(files);
        ...
        setAva(post.userImage ? `http://37.27.29.18:8003/images/${post.userImage}` : userIcon);
        ...
        <div>
            {infoModal && (
                <div className="flex items-center justify-center fixed z-50 inset-0 bg-black/60">
                    <div className="flex flex-col gap-[20px] rounded w-full lg:w-[65%] mx-auto h-[100vh] overflow-y-auto lg:h-[90vh] bg-white">
                        <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                            <div className="lg:w-[50%] w-full lg:h-[90vh]">
                                <div className="flex items-center w-[95%] m-auto pt-[20px] justify-between lg:hidden">
                                    <button className="w-[10%]" onClick={() => setInfoModal(false)}>
                                        {leftIcon}
                                    </button>
                                    <span className="font-bold w-[60%] text-[12px]">Интересное</span>
                                </div>
                                <div className="flex items-center w-[95%] m-auto py-[15px] justify-between lg:hidden">
                                    <div className="flex items-center gap-[10px]">
                                        <Image src={ava} width={30} height={30} className="rounded-full" alt={userName} />
                                    ...
                                    <div className="flex items-center gap-[10px]">
                                        <button
                                            className={`p-[5px] rounded-[5px] font-bold text-[10px] transition-colors ${isFollowing
                                            ...
                                            {file.endsWith(".mp4") ? (
                                                <div className="relative w-full lg:h-screen">
                                                    <video
                                                        ...
                                                        loop
                                                        autoPlay
                                                        onClick={() => { pause ? videoRef.current.play() : videoRef.current.pause(); setPause(!pause); }}
                                                        muted={mute} />
                                                    <div
                            ...
                            <div className="flex flex-col justify-between lg:h-[90vh] w-[95%] gap-2 m-auto lg:p-[20px] lg:w-[50%]">
                                <div className="lg:hidden flex items-center justify-between">
                                    <div className="flex items-center gap-[20px]">
                                        <span className="flex items-center gap-[10px]">{emptyLike}{likes}</span>
                                        <span onClick={() => setCommentModal(true)} className="flex items-center gap-[10px] cursor-pointer">
                                            {commentIcon}{commentCountPost}
                                        </span>
                                    </div>
                                ...
                                <div className="lg:flex hidden items-center gap-[20px]">
                                    ...
                                    <button
                                        className={`cursor-pointer hover:opacity-50 font-bold transition-colors ${isFollowing
                                            ? "text-gray-500"
                                            : "text-blue-500"
                                            }`}
                                        onClick={handleFollow}
                                ...
                                {/* 🗨️ Комментарии */}
                                <div ref={commentsRef} className="lg:flex hidden flex-col gap-5 overflow-y-auto lg:h-screen">
                                                ...
                                                <div className="flex items-start gap-2">
                                                    <div className="flex items-center gap-4">
                                                        <Image
                                                            ...
                                                            src={com.userImage
                                                                ? `http://37.27.29.18:8003/images/${com.userImage}`
                                                                : userIcon}
                                                        />
                                                ...
                                                <div className="ml-[11%]">
                                                    {com.dateCommented && (
                                                        <span className="text-[12px] text-gray-500">
                                                            {new Date(com.dateCommented).toLocaleDateString("ru-RU", {
                                                                day: "numeric",
                                                                month: "long"
                                                            })}
                                                        </span>
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="lg:flex hidden items-center justify-between">
                                        ...
                                        <button onClick={handleFavorite}>
                                            {isFavorite ? solidSavedIcon : savedIconn} {/* переключение */}
                                        </button>
                                    </div>
                                    ...
                                    <div className="lg:flex hidden items-center w-full justify-between">
                                        <div className="relative w-full flex items-center">
                                            <span className="absolute cursor-pointer left-2" onClick={() => setShowPicker((prev) => !prev)}>
                                                {emojiIcon}
                                            </span>
                                            <input
                                                type="text"
                                        ...
                                        <button
                                            ...
                                            className={`px-4 py-2 ${text.length > 0 ? "text-blue-500" : "text-blue-300"}`}
                                            onClick={async () => {
                                                if (!text.trim()) return;
                                                ...
                                                setCommentPost((prev) => ({
                                                    ...prev,
                                                    comments: prev.comments ? [...prev.comments, { comment: text, userName: "Вы", userImage: UserIcon, dateCommented: new Date() }] : [{ comment: text, userName: "Вы", userImage: null, dateCommented: new Date() }]
                                                }));
                        ...
                        <button className="absolute lg:flex hidden top-[20px] text-white right-[20px] z-50" onClick={() => setInfoModal(false)}>
                            {cancelIcon}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col h-full">
                                    ...
                                    <div key={idx} className="flex gap-3 mb-4">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-semibold text-sm">{com.userName}</span>
                                                {com.dateCommented && (
                        ...
                        <div className="p-4 border-t border-gray-200 bg-white">
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    disabled={!text.trim()}
                                    className={`text-sm font-semibold px-3 ${text.trim() ? "text-blue-500" : "text-blue-300"}`}
                                    onClick={async () => {
                                        if (!text.trim()) return;
                                        await addComment(postId, text);
                                        setText("");
            ...
            <div className="grid grid-cols-3 gap-1 p-2">
                {data?.map((post, index) => {
                    ...
                    const fileUrl = `http://37.27.29.18:8003/images/${file}`;
                    return (
                        <div
                            key={index}
                        ...
                        >
                            ...
                            {file?.endsWith(".mp4") ? (
                                <video src={fileUrl} className="w-full h-full object-cover" muted loop />
                            ) : (
                                <Image src={fileUrl} alt="post image" fill className="object-cover" />
                            )}
                            ...
                            <div className="absolute inset-0 hidden group-hover:flex items-center justify-center gap-4 z-10 text-white font-semibold">
                                ...
                                <h1 className="flex items-center gap-[10px]">{commentIcon} {post.commentCount}</h1>
                            </div>
